...
const MovieList = () => {
  ...
  const fetchSampleNames = async () => {
    try {
      const response = await axios.get(`https://movie-library-cl10.onrender.com/api/samples/${email}`);
  ...
  const handleAddToList = async () => {
    try {
      await axios.post('https://movie-library-cl10.onrender.com/api/lists', {
        email,
        movieName,
